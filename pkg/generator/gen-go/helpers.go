...
func goTypeForFieldWithPatternCached(gen *protogen.GeneratedFile, field *protogen.Field, pattern string, cache map[string]string) string {
	...
	} else if field.Desc.HasOptionalKeyword() && field.Desc.Kind() == protoreflect.MessageKind {
		// Handle optional fields (proto3 optional or proto2 optional)
		result = baseType // Already has pointer
	} else {
		result = baseType
	}
